...
class Ease2Transform():
    ...
    def grid_to_map(self, row, col):
        ...
        """
        # x = self.scale_x * col + self.b * row + \
        #     self.scale_x * 0.5 + self.b * 0.5 + self.map_UL_x
        # y = self.d * col + self.scale_y * row + \
        #     self.d * 0.5 + self.scale_y * 0.5 + self.map_UL_y

        ax, ay = self.fwd * (col, row)
        # acol, arow = ~self.fwd * (ax, ay)
        # print("\nTesting row, col = %f, %f" % (row, col), file=sys.stderr)
        # print("    my answer: x, y = %f, %f" % (x, y), file=sys.stderr)
        # print("affine answer: x, y = %f, %f" % (ax, ay), file=sys.stderr)
        # print("affine inverse: row, col = %f, %f" % (arow, acol), file=sys.stderr)
